...
message_queue = asyncio.Queue()  # 创建一个队列用于存储弹幕 | Create a queue to store danmaku messages.

async def main():
    init_session()
    try:
        asyncio.create_task(run_single_client())  # 非阻塞启动监听直播间任务 | Non-blocking start of the live room listening task.
...
class VtuberHandler(blivedm.BaseHandler):
    ...
    def _on_danmaku(self, client: blivedm.BLiveClient, message: web_models.DanmakuMessage):
        ...
        asyncio.create_task(message_queue.put(message.msg))  # 将弹幕消息加入队列 | Add danmaku message to the queue.
            ...
            while True:
                try:
                    ...
                    if "audio" in re_msg:
                        ...
                        re_msg = filtered_msg
                        logger.info(f"收到音频消息 | Received audio message, duration: {re_msg['duration_ms']}ms")

                    # 处理消息结束信号 | Handle conversation end signal.
                    if re_msg.get("type") == "control" and re_msg.get("text") == "conversation-chain-end":
                        msg = await message_queue.get()  # 从队列获取消息 | Get message from the queue.
                        message = {
                            "type": "text-input",
                            "text": msg
